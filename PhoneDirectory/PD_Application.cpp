...
void do_remove_entry(Phone_Directory& the_directory) // Exercise 1.8: worked on by Keenan Robinson
{
	string name;
	cout << "Enter name: ";
	getline(cin, name);
	string deleted_entry = the_directory.remove_entry(name);
	if (deleted_entry != "")
	{
		cout << "the entry for " << name << " has been removed." << endl;
	}
